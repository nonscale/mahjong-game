...
| 스킬 | 설명 | 커버 파일 패턴 |
|------|------|---------------|
| `manage-skills` | 스킬 유지보수 및 등록 관리 | `.agents/skills/manage-skills/*` |
| `verify-implementation` | 통합 검증 프로세스 실행 | `.agents/skills/verify-implementation/*` |
| `verify-data-integrity` | 초장기 시계열 수정주가 무결성 검증 | `MARF/data/raw/full_market_history_master.parquet` |
| `verify-quantum-tactics` | JSON 전술 명세 문법 및 정합성 검증 | `MARF/strategies/*.json` |
| `verify-marf-engine` | Numba JIT 및 엔진 정합성 검증 (Q/Y 확장) | `MARF/src/python/engine/*` |
| `verify-ui-dashboard` | 7분할 대시보드 및 범례 검증 | `MARF/src/python/ui/dashboard.py` |
| `yl-to-python-converter` | YL -> Polars 변환 로직 검증 | `.agents/skills/yl-to-python-converter/*` |
| `yl-to-spot-integrator` | YesSpot 통합 데이터 규약 검증 | `.agents/skills/yl-to-spot-integrator/*` |
